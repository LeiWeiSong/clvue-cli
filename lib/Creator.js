...
class Creator {
    ...
    async initPackageManagerEnv(preset) {
        ...
        // 用于生成 package.json 文件    pkg就是package.json文件的内容
        const pkg = {
            name,
            version: '0.1.0',
            private: true,
    ...
    run(command, args) {
        if (!args) { [command, ...args] = command.split(/\s+/) } // 通过解构赋值的方式 如果没有传入args，就将command按空格分割，第一个是命令，后面的是参数
    ...
    shouldInitGit() {
        ...
        // 项目未初始化 Git 仓库
        return hasProjectGit(this.context)  // 是否已经初始化过git
    }
}
